...
def load_model_cached() -> tf.keras.Model:
    ...
    reliable model loading in Streamlit.
    """
...
def build_quiz_items(source_dir: Path, limit_per_class: int, seed: int) -> List[QuizItem]:
    ...
    limit_per_class : int
        Maximum number of indexed images per class.
    seed : int
    ...
    -------
    List[QuizItem]
        List of quiz items containing image path and label.
    """
...
def preprocess_pil(img: Image.Image) -> np.ndarray:
    ...
    The model already contains its own preprocessing layer, so this function only:
    - converts the image to RGB
    - resizes the image to cfg.PIL_SIZE
    - returns a uint8 array in [0..255]
    """
...
def validate_model_output(probs: np.ndarray) -> None:
    """This function validates that model output matches the configured classes."""
    if probs.ndim != 1:
...
def predict(model: tf.keras.Model, img_arr: np.ndarray) -> Tuple[str, float, np.ndarray]:
    """This function returns the predicted label, confidence, and probability vector."""
...
def probs_table(probs: np.ndarray) -> List[Dict[str, object]]:
    """This function converts probabilities into a table-friendly list."""
    return [
        {"class": class_name, "probability": float(prob)}
        for class_name, prob in zip(cfg.ACTIVE_CLASSES, probs)
    ]
...
def ui_header() -> None:
    """This function renders the page header and basic page settings."""
...
def ui_sidebar() -> str:
    ...
    mode = st.sidebar.radio(
        "Mode",
        ["Single Prediction", "Quiz Mode", "About"],
        index=0,
    )
    st.sidebar.markdown("---")
    st.sidebar.caption(
...
def ui_about() -> None:
    ...
    st.header("About")
    st.write(
        "This application predicts facial emotions from images using a trained deep "
        "learning model based on transfer learning with EfficientNetB0."
    )
...
def ui_single_prediction(model: tf.keras.Model) -> None:
    ...
    st.write(
        "Upload a facial image (JPG/PNG). The model returns the predicted emotion "
        "and the probability distribution."
    )

    file = st.file_uploader(
        "Upload image",
        type=[ext.replace(".", "") for ext in VALID_EXTS],
    )
...
def _select_quiz_source() -> Tuple[Path, str]:
    """This function selects the quiz source folder and returns its path and label."""
    if TEST_DIR.exists():
...
def ui_quiz(model: tf.keras.Model) -> None:
    ...
    st.write("Try to guess the shown emotion. The correct label is revealed after submission.")
    ...
    if not items:
        st.error(
            ...
            "Make sure each class folder contains valid image files."
        )
        return
